...
class WorkoutListAnalysis extends StatefulWidget {
  final List<List<ActivityLog>> logs;

  WorkoutListAnalysis({Key key, this.logs}) : super(key: key);

  @override
  _WorkoutListAnalysisState createState() => _WorkoutListAnalysisState();
}

class _WorkoutListAnalysisState extends State<WorkoutListAnalysis> {
  DateFormat formatterDate = DateFormat('dd MMMM');
  DateFormat formatterTime = DateFormat('jm');
  int tracktype;
  int stattype;
  LinkedHashMap<DateTime, List<ActivityLog>> datelogmap;
  ...
  listToMap(List<List<ActivityLog>> data) {
    ...
    for (var i = 0; i < data.length; i++) {
      for (var j = 0; j < data[i].length; j++) {
        ...
        if (datelogmap.containsKey(correctedDate)) {
          List<ActivityLog> datedata = datelogmap[correctedDate];
  ...
  convergeDataFromMap(int stattype) {
    for (var i = 0; i < datelogmap.entries.length; i++) {
      List<ActivityLog> daylogs = datelogmap.entries.elementAt(i).value;
      double dayValue = 0;
      if (stattype == 0) {
        if (daylogs.length > 0) {
          try {
            dayValue = (daylogs[daylogs.length - 1].caloriesBurnt).toDouble();
          } catch (e) {
            dayValue = 0;
          }
        }
        ...
        if (daylogs.length > 0) {
          try {
            for (ActivityLog e in daylogs) {
              try {
                dayValue += (e.caloriesBurnt).toDouble();
              } catch (e) {
                dayValue += 0;
              }
            }
        ...
        if (daylogs.length > 0) {
          ...
          try {
            for (ActivityLog e in daylogs) {
              try {
                daysum += (e.caloriesBurnt).toDouble();
                ;
              } catch (e) {
                daysum += 0;
              }
