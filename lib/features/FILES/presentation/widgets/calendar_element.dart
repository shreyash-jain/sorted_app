...
class CalendarElement extends StatefulWidget {
  final List<DateTime> selectedMonthEvents;
  final BuildContext context;
  final DateTime selectedDay;
  final Function(DateTime date) moveToMonth;
  final Function(DateTime date) onTapDay;

  const CalendarElement(
      {Key key,
      @required this.selectedMonthEvents,
      @required this.selectedDay,
      @required this.moveToMonth,
      @required this.onTapDay,
      @required this.context})
      : super(key: key);

  @override
  CalendarElementState createState() => CalendarElementState();
}

class CalendarElementState extends State<CalendarElement>
  ...
  void initState() {
    ...
    if (mounted) {
      ...
      _monthColor =
          _monthController.drive(_monthColorTween.chain(_easeInTween));
  ...
  @override
  void didUpdateWidget(CalendarElement oldWidget) {
    print("dddddddd");
    if (_expanded) {
      setState(() {
        calList = [
          Column(
              ...
              mainAxisSize: MainAxisSize.min,
              children:
                  returnRowList(DateTime(showDate.year, showDate.month, 1))),
        ];
      });
    }
        ...
        itemList.add(Expanded(
          child: GestureDetector(
            onTap: () {
              ...
              print(rowValueList[i][j].toString() +
                  "  " +
            ...
            child: Container(
                ...
                decoration: BoxDecoration(
                    ...
                    border: Border.all(
                        color: getColorOfDate(DateTime(
                            start.year, start.month, rowValueList[i][j]))),
                    color: isEventDay(DateTime(
                                start.year, start.month, rowValueList[i][j])) &&
  ...
  Color getColorOfDate(DateTime date) {
    if (isEventDay(date))
      return Theme.of(widget.context).primaryColor;
    else if (areDaysSame(displayDate, date)) {
      return Colors.grey;
    } else if (areDaysSame(widget.selectedDay, date))
      return Colors.black87;
    else
      return Colors.transparent;
  }

  bool isEventDay(DateTime edate) {
    if (widget.selectedMonthEvents.indexOf(edate) != -1) return true;
    return false;
  }
